...
option_list = list(
  make_option(c("-m", "--metadata"), type="character", default="metadata.tsv", 
              ...
              metavar="character"),
  make_option(c("--grouporder"), type="character", default="wt,ko", 
...
grouporder = data.frame(group = stringr::str_split(opt$options$grouporder, ",")[[1]], 
                        stringsAsFactors = F)
...
purrr::walk(pathway_ids$id, function(x, pathways, sample_expression, metadata) {
  ...
  a = pathways %>%
    dplyr::filter(ID == x) %>%
    dplyr::left_join(sample_expression %>% 
  ...
  walk(a$analysis %>% unique, function(y) {
    ...
    ggplot2::ggsave(paste0(output_path, 
                  pathway_name,
                  "_",
                  y,
                  ".png"), # \\/:"*?<>|
