...
all_cp = map(split(payloads_sub, 1:nrow(payloads_sub)) %>% unname %>% map(~ c(.x)),
             ~ do.call(create_subpage_data, list(expr_data = data, sp_info = sp_info, params = .x)))
...
```{r include = F, eval = T}
out = map_if(all_cp, is.not.empty.list, ~ create_subpage_markdown(.x), .else = "")
```
